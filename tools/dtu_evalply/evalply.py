...
def eval_ply(args, scan, oursply):
    if args.mode == 'mesh':
        pbar = tqdm(total=9)
        pbar.set_description('read data mesh')
        data_mesh = o3d.io.read_triangle_mesh(oursply)
        ...
        data_pcd_o3d = o3d.io.read_point_cloud(oursply)
    ...
    data_color = np.tile(G, (data_down.shape[0], 1))
    data_alpha = dist_d2s.clip(max=vis_dist) / vis_dist
    data_color[np.where(inbound)[0][grid_inbound][in_obs]] = R * data_alpha + W * (1 - data_alpha)
    data_color[np.where(inbound)[0][grid_inbound][in_obs][dist_d2s[:, 0] >= max_dist]] = R
    write_vis_pcd(f'{args.vis_out_dir}/vis_{scan:03}_d2s.ply', data_down, data_color)
    stl_color = np.tile(G, (stl.shape[0], 1))
    stl_alpha = dist_s2d.clip(max=vis_dist) / vis_dist
    stl_color[np.where(above)[0]] = R * stl_alpha + W * (1 - stl_alpha)
    stl_color[np.where(above)[0][dist_s2d[:, 0] >= max_dist]] = R
...
if __name__ == '__main__':
    ...
    parser.add_argument('--data', type=str, default=r'E:\hy-tmp\outputs\plys')
    parser.add_argument('--mode', type=str, default='pcd', choices=['mesh', 'pcd'])
    parser.add_argument('--dataset_dir', type=str, default='E:\hy-tmp\SampleSet\MVS Data')
    ...
    for scan in scans:
        ...
        oursply = os.path.join(args.data, "scan{}-ours.ply".format(scan))
        eval_ply(args, scan, oursply)
